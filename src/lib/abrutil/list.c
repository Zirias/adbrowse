...
static void checkAndgrow(List *self, size_t index)
{
    if (index >= self->capacity)
    {
        ...
        self->items = realloc(self->items, self->capacity);
    }
}

static void *stringCloner(const void *string)
{
    return String_copy(string);
}

static size_t stringSize(const void *string)
{
...
{
    return List_create(capacity, free, stringCloner);
}
...
{
    return List_join(self, stringSize, delim, strlen(delim), 0, 0, 1);
}
...
SOEXPORT void ListIterator_delete(ListIterator *self)
{
